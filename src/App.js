...
class MemoryManager {
    ...
    saveConversation(conversationData) {
        try {
            ...
            const newConversation = {
                ...
                userInput: this.sanitizeInput(conversationData.userInput),
                docType: this.sanitizeInput(conversationData.docType),
                generatedContent: conversationData.generatedContent,
    ...
    calculateSimilarity(keywords1, text2) {
        if (!text2 || !keywords1 || keywords1.length === 0) return 0;
...
const EnhancedAiAgentView = () => {
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        const sanitizedInput = memoryManager.sanitizeInput(currentInput);
        if (!sanitizedInput) {
            addMessage('يرجى إدخال نص صحيح.', false);
            return;
        }

        addMessage(sanitizedInput, true);
        setCurrentInput('');

        try {
            if (conversationStage === 'initial') {
                await handleInitialInput(sanitizedInput);
            } else if (conversationStage === 'clarifying') {
                await handleClarificationAnswer(sanitizedInput);
            }
        } catch (error) {
